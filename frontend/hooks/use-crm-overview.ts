...
export function buildCrmOverviewGraph({
  ...
  return companies.map((company) => {
    ...
    const companyActivities = activities.filter((activity) => activityIds.has(String(activity.id)))
...
export function useCrmOverview() {
  ...
  }, [refreshActivities, refreshCalendar, refreshCrm])
  ...
  const companyGraph = React.useMemo(
    () => buildCrmOverviewGraph({ companies, contacts, projects, activities: activities || [], events: events || [] }),
    [activities, companies, contacts, events, projects],
  )
  ...
  const stats = React.useMemo(
    ...
    [activities?.length, companies.length, contacts.length, crmStats, events?.length, projects.length],
  )
