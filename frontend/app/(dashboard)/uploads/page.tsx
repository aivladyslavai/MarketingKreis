...
export default function UploadsPage() {
  ...
  const previewCategoryValues = React.useMemo(() => {
    ...
    const backendValuesByHeader = (preview as any)?.category_values_by_header as Record<string, any[]> | undefined

    // Prefer a full backend-provided list for the currently selected header.
    // This covers all scanned rows, not only the small preview sample.
    const selectedHeaderValues =
      backendValuesByHeader && Array.isArray(backendValuesByHeader[header])
        ? backendValuesByHeader[header]
        : null
    const suggestedHeader = (preview as any)?.suggested_mapping?.category
    const canUseBackendList =
      Array.isArray(selectedHeaderValues) &&
      selectedHeaderValues.length > 0
    const canUseSuggestedBackendList =
    ...
    if (canUseBackendList) {
      for (const raw of selectedHeaderValues || []) {
        const s = String(raw ?? "").trim()
        if (!s) continue
        const key = normalizeCategoryKey(s)
        if (!seen.has(key)) seen.set(key, s)
      }
    } else if (canUseSuggestedBackendList) {
